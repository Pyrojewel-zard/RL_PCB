...
for board in ${EVALUATION_DIR}/* ; do
    ...
    if [ "$SKIP_SA" = false ]; then
        for pcb in $board/sa_pcb/*.pcb; do
            ...
            $BIN/${SA_PLACER} --kicad_pcb $OUTPUT_DIR/$KICAD_PCB -i 300000 --output_dir $OUTPUT_DIR --name $OUTPUT_DIR/$KICAD_PCB --logfile $OUTPUT_DIR/$LOGFILE --ignore_power_nets &> /dev/null
